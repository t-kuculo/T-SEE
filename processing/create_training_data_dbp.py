...
def first_stage_eval_data():
    ...
    {parent_event_dbp_id: {subevent_dbp_id@subevent_link: {"samples":[(link, sentence.text, sentence.links)], "groundtruth":{}}}}
    ...
    for u, filename in enumerate(os.listdir(directory)):
        ...
        for g,js in enumerate(json_list):
            ...
            try:
                parent_event_dbp_id = parent_event_link
            except (KeyError):
                mistakes+=1
                continue
            if parent_event_dbp_id not in events:
                event_dbp_not_in_events+=1
                    ...
                    for link in sentence.links:
                        if "Event" in all_sentence_link_types:
                            if "Event" in link["types"]:
                                ...
                                try:
                                    subevent_dbp_id = subevent_link
                                except (KeyError):
                                    d.append(subevent_dbp_id)
                                    continue
                                i+=1
                                if i%10000==0:
                                    print(i)
                                last_sentence_subevents.append((subevent_dbp_id, subevent_link))
                                if parent_event_dbp_id not in dataset:
                                    dataset[parent_event_dbp_id] = {}
                                if subevent_dbp_id +"@" + subevent_link not in dataset[parent_event_dbp_id]:
                                    dataset[parent_event_dbp_id][subevent_dbp_id +"@" + subevent_link] = {"samples":[], "groundtruth":{}}
                                dataset[parent_event_dbp_id][subevent_dbp_id +"@" + subevent_link]["samples"].append((link, sentence.text, sentence.links))
                                dygiepp_data.append(sentence.text)
                        elif context_window:
                            context_window = 0
                            for subevent in last_sentence_subevents:
                                subevent_dbp_id, subevent_link = subevent
                                link, text, links = dataset[parent_event_dbp_id][subevent_dbp_id +"@" + subevent_link]["samples"][-1]
                                text += sentence.text
                                links += sentence.links
                                dataset[parent_event_dbp_id][subevent_dbp_id +"@" + subevent_link]["samples"][-1] = (link, text, links)
                                dygiepp_data.append(sentence.text)
                            last_sentence_subevents = [] 
    print("all articles:", all)
    print("mistakes:", mistakes)
    print("print not in events:",event_dbp_not_in_events)
...
def load_all_properties():
    '''
    dbp_all_subevents_properties.txt is a file containing all the DBpedia properties of all the subevents in the full raw dataset.
    This function loads the file and creates a dictionary of the following form:
    {subevent_dbp_id: {property: [values]}}
    '''
...
def filter_out_events_without_properties():
    ...
    '''
    d = []
    for event_dbp_id in dataset:
        for subevent in dataset[event_dbp_id]:
            number_of_sentences = len(dataset[event_dbp_id][subevent]["samples"]) 
            try:
                subevent_dbp_id, subevent_link = subevent.split("@")
            except ValueError:
                print(subevent)
            if subevent_dbp_id not in subevents_properties:
                d.append(subevent_dbp_id)
                continue
            subevent_properties = subevents_properties[subevent_dbp_id]
            dataset[event_dbp_id][subevent]["groundtruth"] = subevent_properties
    print(len(d))
...
def extract_data(dataset, types_dbo, subevents_properties, wikidata_ids_to_dbpedia_ids, nlp):
    ''' 
    ...
    - types_dbo (dict): Shelve object mapping types to DBpedia IDs.
    - subevents_properties (dict): Dictionary of properties related to subevents.
    - wikidata_ids_to_dbpedia_ids (dict): Mapping from Wikidata IDs to DBpedia IDs.
    - nlp (spacy.Language): A spaCy NLP model object.

    Returns:
    None; the function saves the processed data into JSON files.
    ...
    - the dbpedia  id of the groundtruth properties

    Then it saves the data in the following form:
    {parent_event_dbp_id: {subevent_dbp_id@subevent_link: [{"sample_event_link":sample_event_link, "untokenized_context":untokenized_context, "tokenized_context":tokenized_context, "sample_groundtruth":sample_groundtruth, "ground_event_types":ground_event_types}]}}
    
    Where sample_event_link is a dictionary containing the following information:
    - the anchor text of the subevent
    - the token range of the anchor text in the context
    - the character range of the anchor text in the context
    - the dbpedia id of the subevent
    ...
    - optionally the dbpedia id of the value of the entity values
    Where ground_event_types is a list of the event classes of the subevent.
    '''
    new_dataset = {}
    for num, event_dbp_id in enumerate(dataset):
        print_progress(num, len(dataset))
        new_dataset = process_event(event_dbp_id, dataset, types_dbo, subevents_properties, wikidata_ids_to_dbpedia_ids, nlp, new_dataset)
...
def process_event(event_dbp_id, dataset, types_dbo, subevents_properties, wikidata_ids_to_dbpedia_ids, nlp, new_dataset):
    ''' 
    Processes all subevents of a given event.

    Parameters:
    - event_dbp_id (str): DBpedia ID of the event being processed.
    ...
    for subevent in dataset[event_dbp_id]:
        new_dataset = process_subevent(subevent, event_dbp_id, dataset, types_dbo, subevents_properties, wikidata_ids_to_dbpedia_ids, nlp, new_dataset)
    return new_dataset

def process_subevent(subevent, event_dbp_id, dataset, types_dbo, subevents_properties, wikidata_ids_to_dbpedia_ids, nlp, new_dataset):
    '''
    ...
    - event_dbp_id (str): DBpedia ID of the parent event.
    ...
    if subevent_dbp_id in types_dbo:
        subevent_types = types_dbo[subevent_dbp_id]
    
    full_url = "<http://dbpedia.org/resource/"+subevent_dbp_id+">"
    ...
    groundtruth_properties = dataset[event_dbp_id][subevent]["groundtruth"]
    ...
    groundtruth_values = {dbp:k for k,v in groundtruth_properties.items() for dbp in v}
    if groundtruth_values == {}:
        print("Skipping subevent with no properties")
        return new_dataset
    
    for sample in dataset[event_dbp_id][subevent]["samples"]:
        new_dataset = process_sample(sample, event_dbp_id, subevent_dbp_id, subevent_types, subevents_properties, groundtruth_values, wikidata_ids_to_dbpedia_ids, nlp, new_dataset)
    return new_dataset

def preprocess_sample_context(sample, subevent_dbp_id, subevents_properties, rx):
    '''
    ...
    - subevent_dbp_id (str): DBpedia ID of the subevent.
    - subevents_properties (dict): Dictionary of properties related to subevents.
...
def process_anchor_text(sample_subevent, doc):
    ...
    Returns:
    dict/bool (False): The updated sample_subevent dictionary with token and character ranges, or False if the anchor text is not found in the context.
    '''
    total = []
    first = False
    try:
...
def process_entities(doc, groundtruth_values, wikidata_ids_to_dbpedia_ids, new_groundtruth):
    '''
    Identifies and processes entities in the sample context, updating the groundtruth data with relevant entity information.
    We do this by looking at the linked entities in the doc and finding the corresponding dbpedia id for each entity.
...
def process_sample(sample, event_dbp_id, subevent_dbp_id, subevent_types, subevents_properties, groundtruth_values, wikidata_ids_to_dbpedia_ids, nlp, new_dataset):
    '''
    Processes individual samples from a subevent to extract and structure relevant information.

    Parameters:
    - sample (tuple): A tuple containing the subevent and its context.
    - event_dbp_id (str): DBpedia ID of the event.
    - subevent_dbp_id (str): DBpedia ID of the subevent.
    ...
    sample, sample_context = preprocess_sample_context(sample, subevent_dbp_id, subevents_properties, rx)
    ...
    new_groundtruth = process_entities(doc, groundtruth_values, wikidata_ids_to_dbpedia_ids, new_groundtruth)
    new_groundtruth = process_literals_and_quantities(doc, groundtruth_values, sample_context, new_groundtruth)

    if event_dbp_id not in new_dataset:
        new_dataset[event_dbp_id] = {}
    if subevent_dbp_id not in new_dataset[event_dbp_id]:
        new_dataset[event_dbp_id][subevent_dbp_id] = []
    if new_groundtruth:
        ...
        new_dataset[event_dbp_id][subevent_dbp_id].append(new_sample)
    return new_dataset
...
def save_data(num, new_dataset):
    ...
    '''
    if num % 100 == 0:
        with open(f"the_data/dbp_data{num}.json", "w") as f:
            json.dump(new_dataset, f)
        new_dataset = {}
    return new_dataset

extract_data(dataset, types_dbo, subevents_properties, wikidata_ids_to_dbpedia_ids, nlp)
